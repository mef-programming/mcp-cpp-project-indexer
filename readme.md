...
\-> find\_symbol("Widget::OnScroll")

\-> read\_symbol(symbolId)
...
list\_changed\_files

get\_file\_change\_hunks(includeIndexedRangeSummary:true, includeSource:false)

get\_file\_change\_hunks(symbolId/dataId, includeSource:true)

read\_symbol/read\_range only when current source behavior is needed
...
python <indexer-root>\\build\_project\_index.py `
...
python <indexer-root>\\code\_index\_mcp\_server.py `
...
&#x20;       "<indexer-root>\\\\code\_index\_mcp\_server.py",
...
find\_symbol -> read\_symbol -> source-grounded answer

```



For best results, give your AI the rules from \[prompt\_template.md](prompt\_template.md).
...
\- \[Recommended AI usage rules](prompt\_template.md)
