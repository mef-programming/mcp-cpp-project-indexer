...
for _support_path in (SRC_ROOT / "indexer", SRC_ROOT / "ui"):
...
def tool_definitions(*, include_orientation: bool = True) -> list[dict[str, Any]]:
        ...
        },
        {
            ...
            "inputSchema": {
                ...
                "properties": {
                    "query": {
                        "type": "string",
                        "description": "Text to search in title, purpose, use-when, boundaries, and map fields.",
                    },
                    "limit": {
                        "type": "integer",
                        "minimum": 1,
    ...
    if not include_orientation:
        tools = [tool for tool in tools if tool.get("name") not in ORIENTATION_TOOL_NAMES]
...
class CodeIndexTools:
    ...
    def has_orientation_evidence(self) -> bool:
        return bool(getattr(self.index, "orientation_nodes", []))

    def get_project_orientation(self, arguments: dict[str, Any]) -> dict[str, Any]:
        max_nodes = clamp_int(arguments.get("maxNodes", 12), minimum=1, maximum=50)
        return self.json_result(arguments, self.index.get_project_orientation(max_nodes=max_nodes))

    def list_orientation_nodes(self, arguments: dict[str, Any]) -> dict[str, Any]:
        limit = clamp_int(arguments.get("limit", 200), minimum=1, maximum=500)
        return self.json_result(arguments, self.index.list_orientation_nodes(limit=limit))

    def get_orientation_node(self, arguments: dict[str, Any]) -> dict[str, Any]:
        path = require_string(arguments, "path")
        result = self.index.get_orientation_node(path)

        if result is None:
            return make_text_result(f"Orientation node not found: {path}", is_error=True)

        return self.json_result(arguments, result)

    def search_orientation(self, arguments: dict[str, Any]) -> dict[str, Any]:
...
class McpServer:
    ...
    def capabilities(self) -> dict[str, Any]:
        ...
        if not self.tools.has_orientation_evidence():
            available_tools -= ORIENTATION_TOOL_NAMES
        categories = {
    ...
    def dispatch(self, method: str, params: dict[str, Any]) -> Any:
        ...
        if method == "tools/list":
            tools = tool_definitions(include_orientation=self.tools.has_orientation_evidence())
        ...
        if method == "tools/call":
            ...
            if tool_name in ORIENTATION_TOOL_NAMES and not self.tools.has_orientation_evidence():
