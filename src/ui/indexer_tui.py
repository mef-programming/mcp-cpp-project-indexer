...
if TEXTUAL_AVAILABLE:
    ...
    class IndexerTuiApp(App):
        ...
        def refresh_status(self) -> None:
            http_status, http_error = read_http_status_with_error(
                self.http_url,
                timeout=1.5,
            ...
            if http_status is not None:
                ...
                self.status_source = "disk"
                self.last_http_error = http_error
        ...
        def update_dashboard(self) -> None:
            ...
            server_status_text = self.http_url if http_connected else "not connected"
            if not http_connected and self.runner_is_server_process():
                server_status_text = "starting / not connected"

            self.query_one("#server", Static).update(
                f"{self.label('Server')}  "
                f"{self.state_value(server_status_text, http_connected)}"
                f"{(' ' + self.error_value(self.last_http_error)) if self.last_http_error else ''}   "
            ...
            self.query_one("#statusbar", Static).update(
                ...
                f"{(' ' + self.label('http=') + self.error_value(self.last_http_error)) if self.last_http_error else ''}"
            )
        ...
        @staticmethod
        def error_value(text: Any) -> str:
            return f"[bold red]{text}[/bold red]"

        def runner_is_server_process(self) -> bool:
            if self.runner.process is None or self.runner.process.poll() is not None:
                return False

            return "code_index_mcp_server.py" in self.runner.last_command.lower()

        def local_process_stats(self) -> dict[str, Any]:
            if not self.runner_is_server_process():
                return {}
